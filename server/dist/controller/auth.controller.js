...
import { generateAccessToken, generateRefreshToken, generateResetPasswwordToken, generateVerifyMailToken } from "../utils/generateTokens.js";
import jwt from 'jsonwebtoken';
import { sendForgotPasswordEmail, sendRegistrationVerifyEmail, sendVerifyEmailMail } from "../utils/mailer.js";
...
export const reqVerifyEmail = async (req, res) => {
    ...
    try {
        ...
        if (!existingUser) {
            return res
                .status(400)
                .json({
                message: "The user with this email does not exists, failed to register",
            });
        }
...
export const verifyEmail = async (req, res) => {
    ...
    try {
        const decoded = jwt.verify(token, process.env.VERIFY_MAIL_TOKEN_SECRET);
